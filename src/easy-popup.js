...
window.EasyPopup = function() {
    ...
    // $(function(){});
    localFunctions.ready(function(){
        var popupSelector = Popup.appendTo ? Popup.appendTo : defaultPopupParent,
        ...
        for (var name in templates) {
            ...
            if (templates[name] === false) {
                ...
                var popupItem = document.querySelectorAll('.easy-popup-tint[data-id="'+Popup.id+'"] > .easy-popup[data-name="'+name+'"]');
                if (popupItem.length) {
                    //popupItem.remove();
        ...
        if (navigator.platform.indexOf('iPad') !== -1 || navigator.platform.indexOf('iPhone') !== -1) {
            if (['object','string'].includes(typeof Popup.hideOnIos)) {
                localFunctions.addClass(document.body, 'easy-popup_ios');
                localFunctions.addClass(document.querySelector('html'), 'easy-popup_ios');

                var hideArr = typeof Popup.hideOnIos == 'string' ? Popup.hideOnIos.split(',') : this.hideOnIos;
    ...
    this.show = function() {
        ...
        var activePopups = document.querySelectorAll('.easy-popup-tint[data-id="'+Popup.id+'"] > .easy-popup.active');
        ...
        if (showParams.name === false) {
            ...
            var tintOnly = document.querySelector('.easy-popup-tint[data-id="'+Popup.id+'"]');
            tintOnly.style.display = 'block';
            localFunctions.addClass(tintOnly, 'active');
            this.isActive = true;
        ...
        if (!localFunctions.hasClass(elem,'active')) {
        ...
        this.closerCallbackMain = function(){
            ...
            this.removeEventListener('click', Popup.closerCallbackMain);
        };
        ...
        var showCompletedCallback = function(){
            ...
            if (Popup.allowCallbackOnEsc) {
                // callback для Esc
                Popup.keydownCallbackEsc = function(e){
                ...
                document.addEventListener('keydown', Popup.keydownCallbackEsc);
            }
            if (Popup.allowCallbackOnEnter) {
                // callback для Enter
                Popup.keydownCallbackEnter = function(e){
                    if (!Popup.isActive || Popup.isLoading) {
    ...
    this.close = function() {
        ...
        */
        ...
        for (var prop in closeParams) {
            ...
            if (arguments.hasOwnProperty(prop))
                closeParams[prop] = arguments[prop];
        }
        ...
        var closeCompleteCallback = function(){
            ...
            localFunctions.removeClass(tint, 'active');
    ...
    this.setLoading = function() {
        ...
        localFunctions.addClass(document.querySelector('.easy-popup-tint[data-id="'+Popup.id+'"]'), 'is_loading');
        ...
        document.querySelector('.easy-popup-tint[data-id="'+Popup.id+'"] > .easy-popup__loading').style.backgroundImage = 'url("'+loadingPic+'")';
        ...
        var setLoadingCallback = function(){
            ...
            if (Popup.closeAfterDelay) {
                Popup.closeAfterDelay = false;
                Popup.unsetLoading();
            }
        };
    ...
    this.unsetLoading = function() {
        ...
        if (Popup.delayedLoading) {
            Popup.closeAfterDelay = true;
            return this;
        }

        // таймаут не установлен, - значит снять класс загрузки
        Popup.isLoading = false;
        localFunctions.removeClass(document.querySelector('.easy-popup-tint[data-id="'+Popup.id+'"]'), 'is_loading');
