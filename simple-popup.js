...
параметры принимаются в виде объекта с указанными ниже свойствами (пример)
{
    styleSheetTitle: '958c1d72380718e5f4b576299fb5ea0c', // title новой таблицы стилей (title тега style) в DOM (используется только для проверки инициализации скрипта, далее всё пишется в уже существующую таблицу стилей)
    speed: 200, // скорость попапа, по умолчанию 150
    closeOnBackgroundClick: true, // закрывать ли попап при клике на фон (.tint); также при вызове попапа (метод show) можно на клик по фону повесить свой callback (backgroundClickCallback)
    closeOnEsc: true, // закрывать ли попап при нажатии Esc; также при вызове попапа (метод show) можно на нажатие Esc повесить свой callback (callbackOnEsc)
    confirmOnEnter: true, // если установлен в true, то при нажатии Enter запускается callback, привязанный к кнопке подтверждения (.confirm_button), если такая кнопка есть; если такой кнопки нет - запускается callback закрытия (closerCallback); также при вызове попапа (метод show) можно на нажатие Enter повесить свой callback (callbackOnEnter)
    templates: {
        confirm: false, // попап-шаблон с именем 'confirm' будет удален из списка по умолчанию
        additional1: '<div class="popup" data-name="additional1">...</div>',
        additional2: '<div class="popup" data-name="additional2">...</div>'
    } // объект с дополнительными шаблонами попапов, которые нужно добавить к уже имеющимся, а также с элементами, которые нужно удалить из имеющихся (например, если передано свойство confirm со значением false, внутри блока .tint производится поиск всех элементов .popup[data-name="confirm"], и все найденные элементы удаляются),
    styles: {
        '.tint .popup': {
            background: '#f00',
            textAlign: 'center',
        ...
        '.popup .my-selector {
            ...
            margin: '20px 0'
        } // добавит дополнительный селектор в таблицу стилей (т.к. его ещё не было)
    ...
    mediaStyles: {
        '@media screen and (max-width: 990px)': {
            '.tint .popup button, .popup input[type="submit"]': {
                background: '#f00' // задаёт красный стиль для кнопок
            }
        }
...
ОСНОВНЫЕ МЕТОДЫ:
popupObj.show (показать попап):
один аргумент - объект с параметрами следующего типа (приведены значения по умолчанию):
{
    ...
    closingBtnCallback: false, // если на .closing_button нужно повесить отдельный callback - передать сюда (если передан - заменит дефолтный)
    confirmBtnCallback: false, // если на .confirm_button нужно повесить отдельный callback - передать сюда (если передан - заменит дефолтный)
