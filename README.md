...
-------------
1) Изменён параметр по умолчанию для ***closingBtnText*** (в попапе `confirm` теперь равен **'Отклонить'**, в остальных случаях равен **'OK'**). Параметр по умолчанию срабатывает также, если передано ***false*** (по строгому сравнению).
2) Изменён параметр по умолчанию для ***confirmBtnText*** (теперь равен **'Подтвердить'**). Параметр по умолчанию срабатывает также, если передано ***false*** (по строгому сравнению).
