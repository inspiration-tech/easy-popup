...
1) Изменены основные CSS-классы попапа:
    ...
    `.tint` -> `.easy-popup-tint`
    
    `.popup` -> `.easy-popup`
    
    `.heading` -> `.easy-popup__heading`
    
    `.body` -> `.easy-popup__body`
    
    `.text` -> `.easy-popup__text`
    
    `.closer` -> `.easy-popup__closer`
    
    `.h_overflow` -> `.easy-popup_overflow`
...
-------------
Класс ***easy-popup_overflow*** не проставляется для устройств с ios
...
8) Пакет переименован из ***simple-popup*** в ***easy-popup***, т.к. на момент публикации имя ***simple-popup*** уже занято в ***npm***.